...
class GameButton extends Component {
  render() {
    if (this.props.illuminatedRed === true) {
      return <button type="button" className="box" id="illuminatedRedBox" />;
    } else if (this.props.illuminatedBlue === true) {
      return <button className="box" id="illuminatedBlueBox" />;
    } else if (this.props.illuminatedYellow === true) {
      return <button className="box" id="illuminatedYellowBox" />;
    } else if (this.props.illuminatedGreen === true) {
      return <button className="box" id="illuminatedGreenBox" />;
    } else
      return (
        <button
          type="button"
...
class SimonGame extends Component {
  ...
  buttonIllumination() {
    if (this.state.gameOn) {
      let illuminatedValue =
        'illuminated' + this.state.lightValues[this.state.count];
  ...
  nextRound() {
    if (this.state.numberOfTurns < 20) {
      ...
      this.state.numberOfTurns === 20 &&
      this.state.lightValues[19] === this.state.playerGuesses[19]
    ) {
      this.setState({
      ...
      <div className="App">
        <div>
          <GameButton
            id="redBox"
            color="Red"
            illuminatedRed={this.state.illuminatedRed}
            onClick={this.handleClick.bind(this)}
          />
          <GameButton
            id="blueBox"
            color="Blue"
            illuminatedBlue={this.state.illuminatedBlue}
            onClick={this.handleClick.bind(this)}
          />
          <GameButton
            id="yellowBox"
            color="Yellow"
            illuminatedYellow={this.state.illuminatedYellow}
            onClick={this.handleClick.bind(this)}
          />
          <GameButton
            id="greenBox"
            color="Green"
            illuminatedGreen={this.state.illuminatedGreen}
            onClick={this.handleClick.bind(this)}
          />
        </div>
        <div>
        ...
        <div>
          <div>
            Turn: {this.state.numberOfTurns}/20
          </div>
          <div>
